...
d = {'img': [], 'exp_val': [], 'clus_pred': [], 'max rec': [], 'treshold': [], 'kern_size': [], 'text': [], 'clus-error': [],'Notes': [],'delta_bacche_abs':[],'delta_bacche':[],'stridex':[],'stridey':[],'final_tresh_coeff':[]}
...
queryes=["the number of raspberries","the number of berries", "a photo of the raspberries","a photo of the berries", "a drone image of the raspberries","a drone image of the berries","the berries on the ground"]  #"the berry", "the berries on the ground","the red berries","the number of red berries","the number of raspberries", "the raspberries"
...
tresh=[0.5,0.2,0.25,0.3,0.35,0.4,0.45,0.55,0.6,0.7]
final_tresh_coeff=tresh
...
stride=[[50,50],["autostride",50],[30,30],[40,40],[60,60]]
...
for strid in stride:
    ...
    for tre in tresh:
        for mxl in mxlen:
            for sqsz1 in sqsz:
                for text in queryes:
                    for final_tresh_coeff1 in final_tresh_coeff:
                        ...
                        for file in os.listdir(dir_path):
                            ...
                            final_tresh_coeff=final_tresh_coeff1,

                            text_add=f"file no:{iterat} of {dirfiles*len(sqsz)*len(tresh)*len(stride)*len(queryes)*len(final_tresh_coeff)}",           #text to add to the print.
                            ...
                            d2 = {'img': [filename], 'exp_val': [ground_truth], 'clus_pred': [len(clst)], 'max rec': [recus], 'treshold': [tre], 'kern_size': [sqz], 'text': [str(text)], 'clus-error': [abs(int(ground_truth)-len(clst))/int(ground_truth)],'Notes': [notes],'delta_bacche_abs':[abs(int(ground_truth)-len(clst))],'delta_bacche':[int(ground_truth)-len(clst)],'stridex':[stridex],'stridey':[stridey],'final_tresh_coeff':[final_tresh_coeff1]}
                        ...
                        if rolling_datasave:    
                            d3 = {'img': ["Error mean"], 'exp_val': [None], 'clus_pred': [None], 'max rec': [None], 'treshold': [None], 'kern_size': [None], 'text': [str(text)], 'clus-error': [df_roll.iloc[:,9].mean()],'Notes': [None],'delta_bacche_abs':[df_roll.iloc[:,11].mean()],'delta_bacche':[int(ground_truth)-len(clst)],'stridex':[stridex],'stridey':[stridey],'final_tresh_coeff':[final_tresh_coeff1]}
